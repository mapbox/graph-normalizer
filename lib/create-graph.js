...
function createGraph(nodes, ways, tile) {
  ...
  var newNodes = nodesToIntersections(nodes, tile);
  var newWays = {};
  var splitWays = [];
  for (var i = 0, n = ways.length; i < n; i++) {
    ...
    way.properties.rawRefs = way.properties.refs.slice();
  ...
  Object.keys(intersections).forEach(function (id) {
    var intersect = intersections[id].properties;

    if (!intersect.terminus && intersect.ways.length < 2) delete intersections[id];
  });
  var graph = {
    ways: newWays,
...
function nodesToIntersections(nodes, tile) {
  ...
  for (var i = 0, n = nodes.length; i < n; i++) {
    ...
    nodes[i].properties.terminus = false;
  ...
  return function (way) {
    ...
    while (i--) {
      ...
      if (intersections[ref] === undefined ||
        ...
        if (i === 0 || i === reflen) intersections[ref].properties.terminus = true;
