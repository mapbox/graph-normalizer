...
module.exports = function (graph) {
  ...
  Object.keys(intersections).forEach(function (id) {
    ...
    for (var k = 0, n = intersections[id].properties.ways.length; k < n; k++) {
      ...
      var index = coordsInArray(intersections[id].geometry.coordinates, way.geometry.coordinates, 1);
      // If the way holds the intersection out of its boundaries, split it.
      if (index > 0 && index < (way.geometry.coordinates.length - 1)) {
        replacements.push({
          oldWay: way,
    ...
    for (var i = 0, m = replacements.length; i < m; i++) {
      ...
      if (newWays.length === 2) {
        ...
        throw new Error('Could not split a way.');
      }
    }
  });
};
