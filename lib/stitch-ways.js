...
var unidirectionalWays = require('./unidirectional-ways');
...
module.exports = function (ways) {
  ...
  // duplicating the ways
  ways = unidirectionalWays(ways);
  ...
  for (var i = 0; i < ways.length; i++) {
    ...
    // hashing sequence of refs
    var md5sum = crypto.createHash('md5');
    md5sum.update(JSON.stringify(way.properties.refs));
  ...
  for (var m = 0; m < segmentList.length; m++) {
    ...
    var md5sum = crypto.createHash('md5');
    md5sum.update(JSON.stringify(combined.properties.refs));
