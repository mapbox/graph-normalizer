...
module.exports = function (wayList, options) {
    ...
    options.mergeAccess = (options.mergeAccess === undefined) ? false : options.mergeAccess;
    ...
    nodes.forEach((ownerIds, node) => {
      ...
      (
          (!options.mergeAccess) &&
        (
            owners[0].properties.access !==
          owners[1].properties.access
        )
      )
        ) nodes.delete(node);
    });
    ...
    while (nodes.size) {
        ...
        if (validMerge) {
            ...
            } else if (Object.hasOwn(opening, 'tunnel')) combined.properties.tunnel = opening.properties.tunnel;

            if (options.mergeJunctions) {
                if (Object.hasOwn(opening, 'junction')) combined.properties.junction = opening.properties.junction;
                else if (Object.hasOwn(closing, 'junction')) combined.properties.junction = closing.properties.junction;
            } else if (Object.hasOwn(opening, 'junction')) combined.properties.junction = opening.properties.junction;

            if (options.mergeAccess) {
                if (Object.hasOwn(opening, 'access')) combined.properties.access = opening.properties.access;
                else if (Object.hasOwn(closing, 'access')) combined.properties.access = closing.properties.access;
            } else if (Object.hasOwn(opening, 'access')) combined.properties.access = opening.properties.access;
