...
module.exports = function (wayList, options) {
  ...
  while (nodes.size) {
    ...
    if (owners.filter(function (owner) { return owner; }).length === 2) {
      ...
      if (opening && closing) {
        // don't merge if access tags are not the same
        if (opening.properties.access !== closing.properties.access) validMerge = false;
      }
    ...
    if (validMerge) {
      ...
      // persist oneway, highway, bridge, tunnel and access tags if they are present
